...
def _remainders(Jacobi_matrix,grid):
...
def quadrature(max_degree,a,b,**kw):
    """Returns accurate grid and weights for specific Gauss-Jacobi quadrature.
        ...
        ----------
        max_degree: int
        Integrates polynomials on (-1,+1) exactly up to degree = 2*max_degree+1.
        a,b: int 
...
def envelope(a,b,a0,b0,z):
    """Returns sqrt( ((1-z)**(a-a0)) * ((1+z)^(b-b0)) / mass(a,b) )
       ...
       Warning: this doesn't work for z = -1, +1.
...
def operator(op,max_degree,a,b,format='csr',rescale=None):
    ...
    if a == np.inf and b == np.inf:
        return Hermite(op,max_degree,format=format,rescale=format)
    
    if a == np.inf or b == np.inf:
        return Laguerre(op,max_degree,min(a,b),format=format,rescale=format)
...
def _sparse_symm_to_banded(matrix):
